...
class SequentialFeatureSelection(FeatureSelection):
    ...
    def __init__(self, clf, scoring, cross_validation_strategy, **kwargs):
        ...
        self.cache_feature_tag = kwargs.get("cache_feature_tag", None)
        ...
        for k_features in range(1, n_features + 1):  # features evaluation loops
            ...
            if k_features == 1:  # initial state
                ...
                if self.can_be_parallelized:
                    ...
                    for feat in feature_list:
                        ...
                        scores_val, scores_test, cm_val, cm_test = self._evaluate(pd.DataFrame(X[feat]), y,
                                                                                  pd.DataFrame(X_test[feat]),
                                                                                  y_test)
                        tmp_res.append(
                            {"features_selected": [feat],
                             ...
                             "val_tp": [int(cm["tp"]) for cm in cm_val],
                             "val_fp": [int(cm["fp"]) for cm in cm_val],
                             "val_tn": [int(cm["tn"]) for cm in cm_val],
                             "val_fn": [int(cm["fn"]) for cm in cm_val],
                             "test_tp": [int(cm["tp"]) for cm in cm_test] if cm_test is not None else np.nan,
                             "test_fp": [int(cm["fp"]) for cm in cm_test] if cm_test is not None else np.nan,
                ...
                if self.can_be_parallelized:
                    ...
                    for feature_set in res_dict[f"{k_features - 1}_features"]["features_selected"].iloc[
                        ...
                        for feat in [f for f in feature_list if f not in feature_set]:
                            ...
                            scores_val, scores_test, cm_val, cm_test = self._evaluate(X[tmp_feature_list], y,
                                                                                      X_test[tmp_feature_list],
                                                                                      y_test)
                            tmp_res.append(
            ...
            if check_for_evaluations_done:
                ...
                if self.cache_feature_path is not None:
                    ...
                    tmp_res.to_csv(f"{self.cache_feature_path}/{k_features}_features_{self.cache_feature_tag}_cached.csv", index=False)
    ...
    def _evaluate(self, X, y, X_test, y_test) -> tuple[list[Any], list[Any] | None, list[Any], list[Any] | None]:
        ...
        for i_train_split, i_val_split in self.cross_validation_strategy.split(X, y):
            ...
            y_pred_val = fitted_classifier.predict(X.iloc[i_val_split])
            tn, fp, fn, tp = confusion_matrix(y[i_val_split], y_pred_val).ravel()
            split_cm_val.append({"tp": tp, "fp": fp, "tn": tn, "fn": fn})
            ...
            if X_test is not None and y_test is not None:
                ...
                split_cm_test.append({"tp": tp, "fp": fp, "tn": tn, "fn": fn})
    ...
    def _evaluate_feature_set(self, feature_set, feat, X, y, X_test, y_test):
        tmp_feature_list = [f for sublist in feature_set + [feat] for f in
                            (sublist if isinstance(sublist, list) else [sublist])]
        scores_val, scores_test, cm_val, cm_test = self._evaluate(X[tmp_feature_list], y, X_test[tmp_feature_list],
                                                                  y_test)
